...
The result type of \ravenjoin will always be the range value (see section \ref{sec:result-types}) as it has the most advantages while it carries the same information as the result of \raptorjoin. 

The selectivity of the filtered joins are small to highlight the efficiency of the algorithm for this case. As listed, the selectivity is set to 0.057\%, 0.059\% and 1.96\% for \textit{GLC2000}, \textit{Treecover} and \textit{Small Woody Features} respectively. Note that these percentages represent how large a portion of the whole image is matched. In practice, the effective percentages are much higher. For example, around 70\% of \textit{GLC2000} is water, but most of our vector datasets are on land. For \textit{Small Woody Features}, these 1.96\% represents all woody features and the remaining part of the images are either water or non-wooded land areas.
...
Table \ref{tab:vector-load-times} shows The time spent loading and building the outer \rtree for the 3 different vector datasets. As expected, the \rtree building takes longer for the larger datasets. The loading time does not follow this pattern, as \textit{Protected Areas} takes longer to read than \textit{Small Woody Features (vector)} even though it is smaller. The problem is with \textit{Protected Areas}, as the read speed is roughly the same for \textit{Boundaries} and \textit{Small Woody Features (vector)},  but around 10 times slower for \textit{Protected Areas}. We do not know what the reason for this is, but we suspect it is caused by the library we use for reading.
