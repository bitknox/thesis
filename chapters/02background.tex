...
At a basic level, the vector data model is defined as points of X and Y coordinate pairs \cite{GIS-Essentials}. Each point represent a location on the earth and therefore a vector dataset can easily cover the whole earth with a few points. This makes the typical file size of vector model much smaller, compared to the raster model. On the other hand, vector data is not good to describe continuous data. 
...
A spatial join can be defined to be a join based on the location of data points of spatial data. More specifically the data is joined by the distance or physical intersection. For raster and vector data, a given polygon is joined with the pixels, that are inside the polygon. There are of course some definitions to be aware of, regarding when to say something is inside. For example, if a pixel is completely within the polygon or if it is just the center point of the pixel.

To join raster and vector data, it is important that the projections of the datasets are the same. The projection information attached to the data, is known as Coordinate Reference System (\textit{CRS}). Since the datasets often are projected differently, it would be inefficient to convert raster to vector or vice versa. The converted dataset would be inflated in size and would require reprojection for each new dataset if the CRS does not match. Raster data is not easily reprojected because each pixel would need to be converted to four points. Instead we want to convert the vector data to the coordinate system of the raster data. This is much cheaper since vector does not have as many points as raster and each point already has a designated location. This comes with the assumption that the lines of the vector data are not very different between the two projections.
...
\subsubsection{\beast}
\beast is a state-of-the-art system, that can join raster and vector data on a distributed level and it utilizes the \raptorjoin algorithm to do this. We are going to compare of our method, with this implementation of \raptorjoin.
...
The final step of \raptorjoin is to read the data and create results. The results consists of tuples (gid, rid, x, y, m) where gid is the id of the geometry. rid is the id of the raster tile. (x, y) is the position within the raster image and m, which holds the value of the pixel.
