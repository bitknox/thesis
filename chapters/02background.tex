...
To join raster and vector data, it is important to convert one of the two, to the other's projection, such that the coordinates match. The projection information attached to the data is known as Coordinate Reference System (CRS) and datasets can be using many different systems. Since it is cumbersome to reproject raster data, we only convert the vector data to the coordinate system of the raster data. This comes with the assumption that the edges of the vector data are not very different in comparison of the two projections. 
