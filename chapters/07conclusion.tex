...
\begin{itemize}
    \item reflection on results (done)
    \item average differences between us and beast (done)
    \item filtering is fast 
    \item result types (done)
    \item low memory consumption (done)
    \item comparison with existing systems (done)
\end{itemize}
...
In our comparison with \raptorjoin, we used 3 different raster datasets. For \textit{GLC2000}, we are 19\% faster on average for filtered joins, and 9\% slower for unfiltered joins. The reason we are slower for unfiltered joins is that we sepnd time building an index of the vector data, something \raptorjoin does not do.
...
Overall, we saw that for filtered joins, \ravenjoin performs way
