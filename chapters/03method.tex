...
To query the vector data efficiently in a given area, a spatial index is used and one of the most common indices, is an r-tree. The \rtree variant improves the search time by minimizing the overlaps of nodes. The vector data is loaded in as polygons that get inserted into the \rtree, one by one. The join benefits from the small overlaps of the directorial nodes of the \rtree. More about this in the evaluation section >>>ref<<<. It is also possible to bulk load the vector data with STR >>>cite<<<, but this creates bad overlaps of the nodes. There is a way to bulk load the data where the overlaps are minimized (OMT >>>cite<<<), but we are not going to evaluate this. In many cases the vector datasets is not large enough to make the bulk loading a larger factor than node overlaps. 
...
\begin{itemize}
\item we use an \rtree instead of a regular r-tree. the \rtree takes longer to build, but saves time in the join because it reduces overlap between nodes. \todo[inline]{run benchmark to ensure an \rtree is actually faster}
...
To achieve out of core execution we partition the raster data into fixed sized squares. This means it is important that the raster data is only read in these same portions for each partition. Otherwise, nothing is saved. Since we know the bounds of the tiles, we can exclude vector data for each tile before the join. The tile size should be a power of two for the \kraster to be optimal. 
\begin{itemize}
...
\subsection{Swift (Data Generation)}

\subsubsection{Raster Data}

\subsubsection{Vector Data}
\subsection{Eagle (Benchmarking)}
