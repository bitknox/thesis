...
In \citetitle{10.1371/journal.pone.0226943}, the join operation described produces a list of probable results and one of definite results. They describe that in order to reduce this to one list of results a \textit{refinement step} is needed where each geometry in the probable result list is examined more closely to find all pixels within them that match the filter. This step is not described further in their paper.
...
Instead, we came up with the idea of making multiple calls to \textit{searchValuesInRanges} where, in each call, at most one range per row is included. This allows us to use a $k$-ary tree that can be built quickly and answer queries in constant time about the horizontal span of ranges within the rows used by a given \kraster node. This span can then be used to determine whether an overlap exists with at least one of the given ranges.
...
With the new approach, we will have to make sure we don't explore parts of the matrix that don't contain any pixel-ranges. To do this, we use the $k$-ary tree explained earlier. Building this tree takes time $t_{tree}$.
